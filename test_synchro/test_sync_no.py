...
def Simulate_PD_Signal(num_samps, rate, f_offset=10e6):
    ...
    # IMPORTANT: Graine FIXE. Le signal simulé sera EXACTEMENT le même.
...
def tx_rx_loopback_nosync(usrp, tx_signal, freq, rate):
    ...
    def rx_worker():
        ...
        # SANS SYNCHRO : On démarre immédiatement
...
def main():
    print("Initialisation du SDR (SANS SYNCHRONISATION PPS)...")
    usrp = uhd.usrp.MultiUSRP()
    
    # On force l'horloge en interne pour être sûr
    ...
    for i in range(10):
        ...
        # On ajoute un délai aléatoire entre chaque tir pour simuler le fait
        # que sans synchro, on démarre à un point aléatoire de la phase 50Hz.
        # delay = np.random.uniform(0.01, 0.05) 
        # time.sleep(0.5 + delay)
        
        rx_signal, t_start = tx_rx_loopback_nosync(usrp, pd_noisy, FREQ, RATE)
    ...
    Plot_PRPD_Multiple(acquisitions, "Test_SANS_Synchro_PPS")
